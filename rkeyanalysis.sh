...
function nonunique {
mysql -h $SERVER -N -B -e"select concat(a.table_schema,'.',a.table_name) from
...
function dupkey() {
	...
	echo '#!/bin/bash' > dupkey.sh
        cat tmpdup2 | grep -v -f nonunique.txt | sed 's/\./ /g' | sed 's/ALTER TABLE //g' | awk '{ print "pt-online-schema-change h=$SERVER,D="$1",t="$2,"--alter \"" substr($0, index($0,$3)) "\" ", "--drop-old-table"}' | sort -r | uniq |sort| uniq |  sed 's/$SERVER/'$SERVER'/g' >> dupkey.sh
	chmod a+x dupkey.sh
...
while true; do
    read -p "Which of the following tools do you wish to run?[1, 2, 3, 4 or (Q)uit] " opt
    case $opt in
        [1]* ) dupkeycheck; nonuniquecheck; dupkey; exit;;
        [2]* )  dupkeycheck; 
		nonuniquecheck; 
		option="big"; 
		big25check; 
